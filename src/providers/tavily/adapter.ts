...
const TAVILY_CAPABILITIES: Capability[] = ["search.web"];

/**
 * Tavily Search provider adapter. [AC4]
 * Maps canonical requests through the Tavily HTTP client and normalizes responses.
 */
export class TavilyAdapter implements ProviderAdapter {
  ...
  constructor(client?: TavilyClient) {
    this.client = client ?? new TavilyClient();
  }
  ...
  async validateCredential(input: unknown): Promise<void> {
    if (typeof input !== "string" || input.length === 0) {
      throw new Error("Tavily credential must be a non-empty string");
    }
  }
  ...
  async execute(
    ...
    if (req.capability !== "search.web") {
      throw new Error(
        `Tavily adapter does not support capability: ${req.capability}`,
      );
    }
  ...
  /** Health check — probes Tavily API with a lightweight request. */
